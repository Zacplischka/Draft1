...
- **Persistence:** session state written to Postgres so a live room can be rehydrated after server restart. Reports are computed on read from persisted ballots — no report table. Host history reads from Postgres.
