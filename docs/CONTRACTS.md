...
type ErrorCode =
  ...
  | 'invalid-input';     // malformed payload (bad enum, score out of 0–100, empty text)
```
...
'solution:combine' { solutionIds: string[] } → ack { solutionId }  // hard-deletes sources;
                                                  // new row text defaults to " / " join, then edit
...
type SessionState = {
  ...
  votingProgress?: { voted: number; total: number };          // counts only — NEVER who
...
```
GET /api/sessions                 → past sessions of the authenticated host
GET /api/sessions/:id/report      → { solutions: [{ id, text, avg, spread }],   // spread = population stddev, 1dp
                                      heatmap: { [dimension]: { [cohort]:
                                        { [solutionId]: number | 'suppressed' } } } }
GET /api/sessions/:id/report.csv  → one row per (solution × dimension × cohort):
                                    ...
                                    plus whole-room rows (dimension = 'all')
```

² `n_voters` is omitted for suppressed cells (printing the count would defeat the suppression).
